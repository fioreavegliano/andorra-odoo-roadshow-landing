...
const EUR = new Intl.NumberFormat("ca-AD", {
  style: "currency",
  currency: "EUR",
});
...
const APPS: AppItem[] = [
  ...
  { key: "quickbooks", name: "Quickbooks", billing: "per_user_month", price: 76 },
  { key: "shopify", name: "Shopify", billing: "fixed_month", price: 79, note: "1% ingressos + 79 €/mes", hasUnmodeledVariable: true },
  { key: "salesforce", name: "Salesforce", billing: "per_user_month", price: 165 },
  { key: "slack", name: "Slack", billing: "per_user_month", price: 14.1 },
  { key: "notion", name: "Notion", billing: "per_user_month", price: 14 },
  { key: "bamboohr", name: "BambooHR", billing: "fixed_month", price: 99, note: "99 €/mes + 10 €/usuari/mes", extraPerUserMonth: 10 },
  ...
  { key: "sharepoint", name: "Sharepoint", billing: "per_user_month", price: 12.5 },
  { key: "lightspeed", name: "Lightspeed", billing: "fixed_month", price: 120, note: "120 €/mes + 29 €/cada 5 usuaris/mes", extraPerBlockMonth: { blockSize: 5, amount: 29 } },
  ...
  { key: "eventbrite", name: "Eventbrite", billing: "mixed", price: 0, note: "1,79 € + 3,7% per tiquet", hasUnmodeledVariable: true },
  { key: "katana", name: "Katana", billing: "fixed_month", price: 359 },
  { key: "booqable", name: "Booqable", billing: "fixed_month", price: 79, note: "79 €/mes + 20 €/usuari/mes", extraPerUserMonth: 20 },
];
...
function annualize(app: AppItem, users: number): number {
  switch (app.billing) {
    ...
    case "fixed_month": {
      ...
      const extraUser = app.extraPerUserMonth ? app.extraPerUserMonth * 12 * users : 0;
      const extraBlock = app.extraPerBlockMonth
        ? Math.ceil(users / app.extraPerBlockMonth.blockSize) * app.extraPerBlockMonth.amount * 12
        : 0;
...
const OdooSavingsSimulator: React.FC<{ className?: string }> = ({ className }) => {
  ...
  const totalApps = selectedApps.reduce((acc, app) => acc + annualize(app, users), 0);
    ...
    <section className={className}>
      <div className="container mb-8 text-center my-20">
        ...
        <p className="mt-2 text-neutral-800">
          Estalvi calculat segons el preu mitjà per usuari de cada aplicació.
        </p>
      </div>
        ...
        <div className="rounded-xl border bg-white p-6 shadow-sm transition-all">
            ...
            {APPS.map((app) => {
              ...
              return (
                ...
                >
                  ...
                  <img src={ICONS[app.key]} alt={app.name} className="h-10 w-10" />
          ...
          <div className="mt-8 text-center">
            ...
            <p className="text-sm text-neutral-500 mt-2">
              Calcula el cost segons la mida del teu equip.
            </p>
          </div>
        </div>

        <aside className="rounded-xl border bg-white p-6 shadow-sm">
          <h3 className="text-xl font-semibold">Programari a substituir</h3>
          <p className="text-sm text-neutral-600 mb-4">
            per a {users} {users === 1 ? "usuari" : "usuaris"} — anualitzat
          </p>
            ...
            {visibleApps.map((app) => {
              ...
              return (
                <li key={app.key} className="py-2 flex justify-between text-sm">
                  ...
                  <span className="tabular-nums font-medium">{EUR.format(annual)}</span>
                </li>
              );
            })}
          </ul>
          ...
          {selectedApps.length > 7 && (
            ...
            >
              {expanded ? (
                <>Veure menys <ChevronUp size={16} /></>
              ) : (
                <>Veure més <ChevronDown size={16} /></>
              )}
            </button>
          )}
          ...
          <div className="mt-4 flex items-center justify-between border-t pt-3">
            ...
            <span className="tabular-nums font-semibold">{EUR.format(totalApps)}</span>
          </div>

          <h4 className="mt-8 text-xl font-semibold">
            Totes les aplicacions d'Odoo
          </h4>
          <div className="flex justify-between">
            <span className="font-semibold">TOTAL</span>
            <span className="tabular-nums font-semibold">{EUR.format(totalOdoo)}</span>
          </div>
          ...
          <div className="mt-8 rounded-xl bg-gradient-to-r from-emerald-50 to-emerald-100 border border-emerald-200 p-5 text-center shadow-inner">
            <div className="text-2xl font-bold text-emerald-800 mt-1 tabular-nums">
              {EUR.format(savings)}{" "}
              <span className="text-lg font-medium">/ any</span>
            </div>
            <p className="text-sm text-emerald-700 mt-1">
              Per gestionar tota la teva empresa amb Odoo.
            </p>
          </div>
        </aside>
      </div>
